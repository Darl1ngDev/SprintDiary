...
首先，非递减顺序，实际就是说一个单调不减，这点卡了我一小会，从示例可以能更快的理解

先将原数组平方，最后从头尾双指针扫描。将更大的一个值插入到`ans`的末尾，随后依次由大的结果到小的结果写入到`ans`中

最后需要注意终止条件时候还没写入，补上`left == right`时的数据即可
